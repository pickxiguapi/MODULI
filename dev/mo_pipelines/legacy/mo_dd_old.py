...
def inference(
    ...
    return_scale = (0.997 ** np.arange(32, dtype=np.float32)).sum()
    val = torch.ones_like(test_prefs)
    condition = torch.cat([test_prefs, val], dim=-1).to(args.device)
    ...
    for i in range(n_episodes):
        ...
        while not np.all(cum_done) and t <= (1000 + 1):
            ...
            # sample trajectories
            prior = torch.zeros((n_envs, horizon, obs_dim), device=device)
            prior[:, 0, :obs_dim] = obs
...
if __name__ == "__main__":
    ...
    if args.nn == "dit":
        nn_diffusion = DiT1d(
            obs_dim , emb_dim=128, d_model=384, n_heads=6, depth=args.depth,
            timestep_emb_type=args.time_emb_type).to(device)
        nn_condition = MLPCondition(
            4, 128, [128, ], nn.SiLU(), 0.25).to(device)
    elif args.nn == "unet":
        raise NotImplementedError

    invdyn = MlpInvDynamic(obs_dim, act_dim, 1024, optim_params={"lr": 5e-4}, device=device)
    invdyn.load(f"results/mo_dd_invdyn/{dataset_name}/invdyn_ckpt100000.pt")

    print(f"======================= Parameter Report of Diffusion Model =======================")
    report_parameters(nn_diffusion)
    print(f"==============================================================================")

    fix_mask = torch.zeros((args.horizon, obs_dim), device=device)
    fix_mask[0, :obs_dim] = 1.
    loss_weight = torch.ones((args.horizon, obs_dim), device=device)
