...
from dev.utils.utils import EmbMLPCondition, MOWeightedCumRewClassifier, MOWeightedRTGClassifier, LRUDatasetCache, ConditionalHalfDiT1d
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pipeline_name", type=str, default="mo_dd_sepguide_pro_rtg")
    ...
    if args.nn == "dit":
        ...
        nn_classifier = ConditionalHalfDiT1d(
            obs_dim, out_dim=pref_dim, cond_dim=pref_dim, emb_dim=args.emb_dim, d_model=args.d_model//2, n_heads=args.n_heads, depth=max(args.depth//2, 1),
            timestep_emb_type=args.time_emb_type).to(args.device)
    ...
    classifier = MOWeightedRTGClassifier(nn_classifier, device=args.device)
