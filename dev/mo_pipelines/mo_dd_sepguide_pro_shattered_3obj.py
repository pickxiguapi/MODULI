...
from dev.dataset.peda_mujoco_dataset import PEDAMuJoCoDataset, ShatteredPEDAMuJoCoDataset, ShatteredPEDAMuJoCoDataset3Obj
...
hv = InnerHyperVolume([0, 0, 0])
from dev.utils.utils import EmbMLPCondition, MOWeightedCumRewClassifier, LRUDatasetCache, ConditionalHalfDiT1d
import lora_pytorch
...
def inference(
    ...
    eval_prefs = torch.tensor(eval_prefs, dtype=torch.float32).to(args.device)
    args.num_prefs =325
    ...
    condition_cfg = eval_prefs.clone().to(args.device)
    condition_cg = eval_prefs.clone().to(args.device)
        ...
        while idx < args.num_prefs:
            ...
            while not np.all(cum_dones) and t <= 500:
                ...
                traj, log = agent.sample(
                    ...
                    condition_cfg=condition_cfg[idx:idx + current_num_envs].repeat(args.n_elites, 1), w_cfg=args.w_cfg,
                    condition_cg=condition_cg[idx:idx + current_num_envs].repeat(args.n_elites, 1), w_cg=args.w_cg, temperature=args.temperature)

                # select the best plan
                logp = log["log_p"].view(args.n_elites, current_num_envs, -1).sum(-1)
                idx_act = logp.argmax(0)
                nxt_obs = traj.view(args.n_elites, current_num_envs, args.horizon, -1)[idx_act, torch.arange(current_num_envs), 1, :obs_dim]
                act = invdyn.predict(obs, nxt_obs).cpu().numpy()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pipeline_name", type=str, default="mo_dd_sepguide_pro_shattered")
    ...
    parser.add_argument("--eps", type=float, default=3e-3,
    ...
    parser.add_argument("--num_prefs", type=int, default=325,
    ...
    dataset = ShatteredPEDAMuJoCoDataset3Obj(
    ...
    if args.nn == "dit":
        ...
        nn_classifier = ConditionalHalfDiT1d(
            obs_dim, out_dim=1, cond_dim=pref_dim, emb_dim=args.emb_dim, d_model=args.d_model//2, n_heads=args.n_heads, depth=max(args.depth//2, 1),
            timestep_emb_type=args.time_emb_type).to(args.device)
        nn_condition = EmbMLPCondition(
            pref_dim, 128, [128, ], nn.SiLU(), 0.25, args.cond_emb_type).to(args.device)
    elif args.nn == "unet":
    ...
    classifier = MOWeightedCumRewClassifier(nn_classifier, device=args.device)
    ...
    if args.mode == "train":
        ...
        for batch in loop_dataloader(dataloader):
            ...
            if step <= 500_000:
                log["avg_loss_classifier"] += agent.update_classifier(obs, [pref, val])['loss']
            ...
            if (step + 1) % 100_000 == 0:
                ...
                if eval_hv.mean() > best_hv:
                    ...
                    ax.scatter(dataset.seq_rew[::50, :, 0].sum(1), dataset.seq_rew[::50, :, 1].sum(1), dataset.seq_rew[::50, :, 2].sum(1), c="y", alpha=0.5, edgecolor='none')
                    ax.scatter(eval_rewards[:, :, 0].mean(0), eval_rewards[:, :, 1].mean(0), eval_rewards[:, :, 2].mean(0), c="r", edgecolor='none')
