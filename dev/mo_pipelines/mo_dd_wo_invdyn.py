    ...
    for ep in range(args.num_episodes):
        ...
        while idx < args.num_prefs:
            ...
            while not np.all(cum_dones) and t <= 500:
                # normalize obs
                obs = torch.tensor(normalizer["state"].normalize(obs), device=args.device, dtype=torch.float32)
                ...
                with torch.no_grad():
                    # act_1 = invdyn.predict(obs, traj[:, 1, :obs_dim]).cpu().numpy()
                    act_2 = traj[:, 0, obs_dim:].cpu().numpy()
                    unnormed_act = normalizer["action"].unnormalize(act_2)

                # tmp = unnormed_act-act_1
                # step env
                # obs, _, done, info = env.step(act)
                obs, _, done, info = env.step(unnormed_act)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pipeline_name", type=str, default="mo_dd_wo_invdyn")
