...
def inference(
    ...
    eval_prefs = torch.tensor(eval_prefs).to(args.device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pipeline_name", type=str, default="mo_diffuser_prefguide")
    ...
    classifier = MOPrefClassifier(nn_classifier, device=args.device, scale_param=1.)
    ...
    if args.mode == "train":
        ...
        np.save(save_path + f"eval_reward_{args.eval_name}.npy", eval_rewards)
        np.save(save_path + f"eval_pref_{args.eval_name}.npy", eval_prefs)
        ...
        fig.savefig(save_path + f"eval_plot_{args.eval_name}.pdf")