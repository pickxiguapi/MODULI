    ...
    for ep in range(args.num_episodes):
        ...
        while idx < args.num_prefs:
            ...
            while not np.all(cum_dones) and t <= 500:
                ...
                # sample trajectories
                prior = torch.zeros((current_num_envs, args.horizon, obs_dim + act_dim + pref_dim), device=args.device)
                prior[:, 0, :obs_dim] = obs
                prior[:, :, -pref_dim:] = 1.
                ...
                act = traj[:, 0, obs_dim:obs_dim + act_dim].cpu().numpy()
...
if __name__ == "__main__":
    ...
    if args.nn == "dit":
        nn_diffusion = DiT1d(
            obs_dim + act_dim + pref_dim, emb_dim=args.emb_dim, d_model=args.d_model, n_heads=args.n_heads, depth=args.depth,
    ...
    fix_mask = torch.zeros((args.horizon, obs_dim + act_dim + pref_dim), device=args.device)
    fix_mask[0, :obs_dim] = 1.
    fix_mask[:, -pref_dim:] = 1.
    loss_weight = torch.ones((args.horizon, obs_dim + act_dim + pref_dim), device=args.device)
