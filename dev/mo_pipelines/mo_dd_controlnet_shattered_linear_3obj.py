...
from dev.utils.diffusion_utils import ContinuousVPSDEControlNet3Obj, ContinuousVPSDEControlNet3ObjLinear
...
def process_controlnet_weight(pref: torch.Tensor, cond: torch.Tensor, slider: bool = False):
    ...
    for i in range(cond.shape[0]):
        pref_cond_distance = torch.norm(pref - cond[i, :], dim=-1)
        ...
        if nearest_pref_distance > 0.01 * 1.414 * 3 and slider:
            output_cond[i] = nearest_pref
            output_weight[i] = cond[i, :] - nearest_pref
        else:
            output_cond[i] = cond[i, :]
            output_weight[i] = 0
...
def inference(
    ...
    if uniform:
        ...
        eval_prefs = torch.tensor(eval_prefs, dtype=torch.float32)
        args.num_prefs =325
    else:
...
if __name__ == "__main__":
    ...
    agent = ContinuousVPSDEControlNet3ObjLinear(
