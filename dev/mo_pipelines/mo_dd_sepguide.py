...
def inference(
    ...
    condition_cg = val.clone().to(args.device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pipeline_name", type=str, default="mo_dd_sepguide")
    ...
    parser.add_argument("--num_prefs", type=int, default=40,
    ...
    if args.nn == "dit":
        ...
        nn_classifier = HalfDiT1d(
            obs_dim , out_dim=pref_dim, emb_dim=args.emb_dim, d_model=args.d_model//2, n_heads=args.n_heads, depth=max(args.depth//2, 1),
            timestep_emb_type=args.time_emb_type).to(args.device)
    ...
    if args.mode == "train":
        ...
        for batch in loop_dataloader(dataloader):
            ...
            if step <= 500_000:
                log["avg_loss_classifier"] += agent.update_classifier(obs, val)['loss']
