...
def inference(
    ...
    # else:
    #     eval_prefs = torch.linspace(dataset.seq_pref[:, 0].min(), dataset.seq_pref[:, 0].max(), args.num_prefs).reshape(-1, 1)
    ...
    condition = torch.cat([eval_prefs, val], dim=-1).to(args.device)
        ...
        while idx < args.num_prefs:
            ...
            while not np.all(cum_dones) and t <= 500:
                ...
                traj, log = agent.sample(
                    prior, n_samples=current_num_envs, sample_steps=args.sample_steps, sample_step_schedule=args.sample_step_schedule, use_ema=True, solver=args.solver,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pipeline_name", type=str, default="mo_dd_trivial")
    ...
    parser.add_argument("--avg", type=str2bool, nargs='?', default=False, choices=[True, False], 
    ...
    parser.add_argument("--normalize_rewards", type=str2bool, nargs='?', default=False, choices=[True, False],
    ...
    if args.mode == "train":
        ...
        R_max = torch.from_numpy(np.cumsum(dataset.seq_rew[:, ::-1], axis=1).max(axis=0).max(axis=0)).to(args.device)
        R_min = torch.from_numpy(np.cumsum(dataset.seq_rew[:, ::-1], axis=1).min(axis=0).min(axis=0)).to(args.device)
        R_range = R_max-R_min
        ...
        for batch in loop_dataloader(dataloader):
            ...
            val = batch["score"].mean(axis=1).to(args.device)
            pref = batch["pref"].to(args.device)

            val = (val - R_min) / R_range
