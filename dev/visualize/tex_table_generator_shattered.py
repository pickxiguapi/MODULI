...
def compute_match(rewards: np.ndarray, prefs: np.ndarray, dataset_name: str):
    front = nn.Sequential(
        nn.Linear(2, 128),
        nn.Mish(),
        nn.Linear(128, 512),
    ...
    front.load_state_dict(torch.load(f"results/front_approx/{dataset_name}/approx_ckpt100000.pt"))
    exp_rewards = front(torch.tensor(prefs).float()).detach().numpy()
    # gap = (((exp_rewards - rewards) / exp_rewards) * prefs).sum(-1) * 100
    gap = (rewards * prefs).sum(-1)

    real_pref = rewards / rewards.sum(-1, keepdims=True)
    real_pref = np.clip(real_pref, 0, 1)
    pref_dis = np.power(real_pref - prefs, 2)[:, 0] * 100
    return gap.mean(), pref_dis.mean()
...
# standard
# datasets = [
#     [f"Diffusion", f"results/mo_dd/MO-{env}_50000_amateur_uniform/eval_standard/11M/rewards/standard.npy"] for env in envs 
# ] + [
#     [f"Diffusion", f"results/mo_dd/MO-Hopper-v3_50000_amateur_uniform/eval_standard/22M/eval_reward_.npy"]
# ] + [
#     [f"Diffusion", f"results/mo_dd/MO-{env}_50000_expert_uniform/eval_standard/11M/rewards/standard.npy"] for env in envs 
# ] + [
#     [f"Diffusion", f"results/mo_dd/MO-Hopper-v3_50000_expert_uniform/eval_standard/22M/eval_reward_.npy"]
# ]
...
# shatter
datasets = [
    [f"Non-Slider", f"results/mo_dd_shattered/MO-{env}_50000_{dataset_type}_uniform/eval_quick/11M_30/shatter/rewards/non_slider.npy"] for dataset_type in dataset_types for env in envs 
] + [
    [f"Slider", f"results/mo_dd_shattered/MO-{env}_50000_{dataset_type}_uniform/eval_quick/11M_30_controlnet/shatter/rewards/slider.npy"] for dataset_type in dataset_types for env in envs 
]

# side
datasets = [
    [f"Non-Slider", f"results/mo_dd_shattered/MO-{env}_50000_{dataset_type}_uniform/eval_quick/11M_S30/side/rewards/non_slider.npy"] for dataset_type in dataset_types for env in envs 
] + [
    [f"Slider", f"results/mo_dd_shattered/MO-{env}_50000_{dataset_type}_uniform/eval_quick/11M_S30_controlnet/side/rewards/slider.npy"] for dataset_type in dataset_types for env in envs 
]
...
use_baselines_hv = [["B"], ["MORvS(P)"]]
use_baselines_hv = []
use_baselines_sp = [["MODT(P)", "MORvS(P)", "BC(P)", "CQL(P)"]]
use_baselines_sp = []
use_baselines_wreward = []
use_baselines_prefdis = []
...
for i, [algo_name, path] in enumerate(datasets):
    ...
    eval_pref = np.load(path.replace("rewards", "prefs"))
    ...
    weighted_reward = np.array([compute_match(eval_data[i], eval_pref[i], dataset_name)[0] for i in range(eval_data.shape[0])])
    pref_dis = np.array([compute_match(eval_data[i], eval_pref[i], dataset_name)[1] for i in range(eval_data.shape[0])])
    ...
    table_prefdis[dataset_name][algo_name] = [round(pref_dis.mean(), 2), round(pref_dis.std(), 2)]
    table_wreward[dataset_name][algo_name] = [round(weighted_reward.mean(), 2), round(weighted_reward.std(), 2)]
...
width = sum([len(i) for i in use_baselines_prefdis]) + len(use_algos)
height = len(use_datasets)

if std:
    head = "".join(["p{0.1\\textwidth}"*len(i) + "|" for i in use_baselines_prefdis]) + "p{0.1\\textwidth}"*len(use_algos)
else:
    head = "".join(["p{0.08\\textwidth}"*len(i) + "|" for i in use_baselines_prefdis]) + "p{0.08\\textwidth}"*len(use_algos)
env = " & ".join(["Environments"] + [item for sublist in use_baselines_prefdis for item in sublist] + use_algos)
...
for idx_d, dataset in enumerate(use_datasets):
    ...
    for j in use_baselines_prefdis:
        for k in j:
            if k != "B":
                if baseline_table_prefdis[dataset][k][0] < line_min:
                    line_min = baseline_table_prefdis[dataset][k][0]
                    line_min_std = baseline_table_prefdis[dataset][k][1]
                elif baseline_table_prefdis[dataset][k][0] == line_min:
                    line_min_std = min(line_min_std, baseline_table_prefdis[dataset][k][1])
    
    for j in use_algos:
        if table_prefdis[dataset][j][0] < line_min:
            line_min = table_prefdis[dataset][j][0]
            line_min_std = table_prefdis[dataset][j][1]
        elif table_prefdis[dataset][j][0] == line_min:
            line_min_std = min(line_min_std, table_prefdis[dataset][j][1])
    
    for j in use_baselines_prefdis:
        for k in j:
            if std:
                if baseline_table_prefdis[dataset][k][0] <= line_min and k != 'B':
                    body += "\\textbf{" + f"{baseline_table_prefdis[dataset][k][0]:.2f}±" + '.' + str(round(baseline_table_prefdis[dataset][k][1], 2)) + "}" + " & "
                else:
                    body += f"{baseline_table_prefdis[dataset][k][0]:.2f}±" + '.' + str(round(baseline_table_prefdis[dataset][k][1], 2)) + " & "
            else:
                if baseline_table_prefdis[dataset][k][0] <= line_min and k != 'B':
                    body += "\\textbf{" + f"{baseline_table_prefdis[dataset][k][0]:.2f}" + "}" + " & "
                else:
                    body += f"{baseline_table_prefdis[dataset][k][0]:.2f}" + " & "
    
    for idx_j, j in enumerate(use_algos):
        if table_prefdis[dataset].get(j) is not None:
            if std:
                if table_prefdis[dataset][j][0] <= line_min:
                    body += "\\textbf{" + f"{table_prefdis[dataset][j][0]:.2f}±" + '.' + str(round(table_prefdis[dataset][j][1], 2)) + "}"
                else:
                    body += f"{table_prefdis[dataset][j][0]:.2f}±" + '.' + str(round(table_prefdis[dataset][j][1], 2))
            else:
                if table_prefdis[dataset][j][0] <= line_min:
                    body += "\\textbf{" + f"{table_prefdis[dataset][j][0]:.2f}" + "}"
                else:
                    body += f"{table_prefdis[dataset][j][0]:.2f}"
        else:
            body += "/"
...
width = sum([len(i) for i in use_baselines_wreward]) + len(use_algos)
height = len(use_datasets)

if std:
    head = "".join(["p{0.1\\textwidth}"*len(i) + "|" for i in use_baselines_wreward]) + "p{0.1\\textwidth}"*len(use_algos)
else:
    head = "".join(["p{0.08\\textwidth}"*len(i) + "|" for i in use_baselines_wreward]) + "p{0.08\\textwidth}"*len(use_algos)
env = " & ".join(["Environments"] + [item for sublist in use_baselines_wreward for item in sublist] + use_algos)
...
for idx_d, dataset in enumerate(use_datasets):
    ...
    for j in use_baselines_wreward:
        for k in j:
            if k != "B":
                if baseline_table_wreward[dataset][k][0] > line_max:
                    line_max = baseline_table_wreward[dataset][k][0]
                    line_max_std = baseline_table_wreward[dataset][k][1]
                elif baseline_table_wreward[dataset][k][0] == line_max:
                    line_max_std = max(line_max_std, baseline_table_wreward[dataset][k][1])
    
    for j in use_algos:
        if table_wreward[dataset][j][0] > line_max:
            line_max = table_wreward[dataset][j][0]
            line_max_std = table_wreward[dataset][j][1]
        elif table_wreward[dataset][j][0] == line_max:
            line_max_std = max(line_max_std, table_wreward[dataset][j][1])
    
    for j in use_baselines_wreward:
        for k in j:
            if std:
                if baseline_table_wreward[dataset][k][0] >= line_max and k != 'B':
                    body += "\\textbf{" + f"{baseline_table_wreward[dataset][k][0]:.2f}±" + '.' + str(round(baseline_table_wreward[dataset][k][1], 2)) + "}" + " & "
                else:
                    body += f"{baseline_table_wreward[dataset][k][0]:.2f}±" + '.' + str(round(baseline_table_wreward[dataset][k][1], 2)) + " & "
            else:
                if baseline_table_wreward[dataset][k][0] >= line_max and k != 'B':
                    body += "\\textbf{" + f"{baseline_table_wreward[dataset][k][0]:.2f}" + "}" + " & "
                else:
                    body += f"{baseline_table_wreward[dataset][k][0]:.2f}" + " & "
    
    for idx_j, j in enumerate(use_algos):
        if table_wreward[dataset].get(j) is not None:
            if std:
                if table_wreward[dataset][j][0] >= line_max:
                    body += "\\textbf{" + f"{table_wreward[dataset][j][0]:.2f}±" + '.' + str(round(table_wreward[dataset][j][1], 2)) + "}"
                else:
                    body += f"{table_wreward[dataset][j][0]:.2f}±" + '.' + str(round(table_wreward[dataset][j][1], 2))
            else:
                if table_wreward[dataset][j][0] >= line_max:
                    body += "\\textbf{" + f"{table_wreward[dataset][j][0]:.2f}" + "}"
                else:
                    body += f"{table_wreward[dataset][j][0]:.2f}"
        else:
            body += "/"
