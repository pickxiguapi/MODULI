...
# for root, dirs, files in os.walk("experiment_runs/uniform"):
...
#         # print(ema_pref[:, 0]-np.linspace(0, 1, ema_pref.shape[0]))

#         # table_hv[dataset_name][algo_name] = [round(np.array([HyperVolume.compute(eval_data[:, i, :]) for i in range(eval_data.shape[1])]).mean()/scale_hv[dataset_name], 2), round(np.array([HyperVolume.compute(eval_data[:, i, :]) for i in range(eval_data.shape[1])]).std()/scale_hv[dataset_name], 2)]
...
# standard
datasets = [
    [f"MODULI", f"results/mo_dd/MO-{env}_50000_{dataset_type}_uniform/eval_standard/11M_15ep/rewards/500k.npy"] for env in envs for dataset_type in dataset_types
] + [
    [f"MODULI", f"results/mo_dd/MO-Hopper-v3_50000_{dataset_type}_uniform/eval_standard/22M/eval_reward_.npy"] for dataset_type in dataset_types
]
...
# ] + [
#     [f"CFG+CG", f"results/mo_dd_sepguide_pro_shattered/MO-{env}_50000_{dataset_type}_uniform/eval_standard/11M_30/rewards/200k.npy"] for env in envs for dataset_type in dataset_types
# ] + [
#     [f"CG", f"results/mo_diffuser_prefguide_shattered/MO-{env}_50000_{dataset_type}_uniform/eval_standard/11M_30/rewards/200k.npy"] for env in envs for dataset_type in dataset_types 
# ] + [
...
# ] + [
#     [f"CFG+CG", f"results/mo_dd_sepguide_pro_shattered/MO-Hopper-v3_50000_{dataset_type}_uniform/eval_standard/11M_30/rewards/200k.npy"] for dataset_type in dataset_types
# ] + [
#     [f"CG", f"results/mo_diffuser_prefguide_shattered/MO-Hopper-v3_50000_{dataset_type}_uniform/eval_standard/11M_30/rewards/200k.npy"] for dataset_type in dataset_types
# ]

# normalization
# datasets = [
#     [f"MODULI", f"results/mo_dd/MO-{env}_50000_{dataset_type}_uniform/eval_standard/11M/rewards/500k.npy"] for dataset_type in dataset_types for env in envs 
# ] + [
#     [f"MODULI w/o neighborhood normalization", f"results/mo_dd_wo_norm/MO-{env}_50000_{dataset_type}_uniform/eval_standard/22M/rewards/standard.npy"] for dataset_type in dataset_types for env in envs 
# ] + [
...
#     [f"MODULI", f"results/mo_dd/MO-Hopper-v3_50000_{dataset_type}_uniform/eval_standard/22M/eval_reward_.npy"] for dataset_type in dataset_types
# ] + [
#     [f"MODULI w/o neighborhood normalization", f"results/mo_dd_wo_norm/MO-Hopper-v3_50000_{dataset_type}_uniform/eval_standard/11M/rewards/200k.npy"] for env in envs for dataset_type in dataset_types
# ] + [
#     [f"MODULI global normalization", f"results/mo_dd/MO-Hopper-v3_50000_{dataset_type}_uniform/eval_standard/11M_global_norm/rewards/200k.npy"] for env in envs for dataset_type in dataset_types
# ] + [
#     [f"MODULI", f"results/mo_dd/MO-Hopper-v3_50000_{dataset_type}_uniform/eval_standard/22M/eval_reward_.npy"] for dataset_type in dataset_types
# ] + [
#     [f"MODULI w/o neighborhood normalization", f"results/mo_dd_wo_norm/MO-Hopper-v3_50000_{dataset_type}_uniform/eval_standard/11M/rewards/200k.npy"] for dataset_type in dataset_types
# ] + [
#     [f"MODULI global normalization", f"results/mo_dd/MO-Hopper-v3_50000_{dataset_type}_uniform/eval_standard/11M_global_norm/rewards/200k.npy"] for dataset_type in dataset_types
# ]
...
use_baselines_hv = [["B"], ["CQL(P)"]]
# use_baselines_hv = []
use_baselines_sp = [["B"], ["CQL(P)"]]
# use_baselines_sp = []
...
for idx, [algo_name, path] in enumerate(datasets):
    ...
    if "v2" in path:
        eval_data = eval_data.reshape((5, 3, -1, 2)).mean(axis=0)
...
for idx_d, dataset in enumerate(use_datasets):
    ...
    # HV
    if idx_d == 0:
        body += "\\multirow{15}*{\\rotatebox{90}{Amateur}} & "
    elif idx_d == 6:
        body += "\\multirow{15}*{\\rotatebox{90}{Expert}}  & "
    else:
        body += "~                                       & "

    # dataset = "MO-{dataset_name}_50000_{dataset_type}_uniform"
    body += "\\multirow{2}*{" + f"{dataset.split('-')[1] + ('-3obj' if dataset.split('-')[2].split('_')[0]=='v3' else '')}" + "}" + f"{' '*(5*(dataset.split('-')[2].split('_')[0]!='v3')+6-len(dataset.split('-')[1]))} & " + f"HV ($\\times 10^{{{int(log10(scale_hv[dataset]))}}}$) & "
...
\\begin{{table*}}[htb]
  ...
  \\label{{sample-table}}
  \\centering
  \\begin{{tabular}}{{p{{0.03\\textwidth}}p{{0.1\\textwidth}}p{{0.1\\textwidth}}||p{{0.05\\textwidth}}|{head}}}
    \\toprule
    {env} \\\\
    \\midrule
    {body}
